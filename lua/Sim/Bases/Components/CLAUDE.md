...
Components are **pure data holders** with narrow scope. They carry one aspect of base state (sections, build sites, economy plans, etc.) and expose accessors over it. They do **not** orchestrate, mirror to the brain, or call sibling components — that's [`JoeBase`](../JoeBase.lua)'s job (see [`../CLAUDE.md`](../CLAUDE.md)).
...
- [`JoeBaseBuildQueueComponent.lua`](JoeBaseBuildQueueComponent.lua) — three queues for the build-job lifecycle: `Pending` (unclaimed), `Active` (engineer working), `Complete` (built, unit alive). `JoeBuildJobSpec` is the immutable request — single `Identifier`, optional `LocationHint` / `Priority` / `MaxAssistants` / `DelayPredicate`. `JoeBuildJob` wraps it with runtime state (engineers list, build site, unit, delayed flag). Lifecycle helpers `PushJob` / `ClaimJob` / `RegisterBuildSite` / `RegisterUnit` / `CompleteJob` / `FailJob`; assistant API `JoinAsAssistant` / `LeaveJob` / `FindAssistTarget`. A forked `PollLoop` thread (cadence `PollInterval` ticks) runs `RefreshDelays` over `Pending` and re-queues `Complete` jobs whose units have been destroyed.
