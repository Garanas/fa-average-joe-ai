...
- [`JoeBaseProductionQueueComponent.lua`](JoeBaseProductionQueueComponent.lua) — same lifecycle shape as the construction queue, but factory-driven: each job is a single mobile unit produced by one factory. `JoeProductionJobSpec` is role-shaped — `Category` (the unit role, e.g. `MOBILE * LAND * DIRECTFIRE`), `TechPreference` (a `TechCategory` string used by the factory behavior for ordering, not as a hard filter), optional `LocationHint` / `Priority` / `DelayPredicate`. `CollectEligibleFor(factory, cache)` returns the jobs the factory can actually build (faction × tech intersection); selection sits in the factory behavior. `JoeProductionJob` tracks the claiming `Factory` and the produced `Unit`. No build site (factories produce in their own internal queue) and no assistant slot (factory-assist by engineers is a different mechanic, not tracked here). Validators mirror the construction queue and are driven from the same place.
